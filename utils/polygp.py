...
def _calculate_bic2(n, llh, cov, alpha):

    eigenvals = la.eig(cov)[0] # eigenvalues
    k = np.sum(eigenvals / (eigenvals + alpha)) # here alpha is interpreted as the variance or noise added to the diagonal of the kernel matrix
    bic = np.real(-2 * llh + k * np.log(n)) #num_params*log(N) - 2*log(L)
    return bic
